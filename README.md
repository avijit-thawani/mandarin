...
- **Never-quizzed words jump the queue**: `selectWeakConcepts` adds a large bonus (`UNSEEN_BOOST`) to any word with zero attempts across the focused modalities, so a word you just added leads the next session and then joins the normal rotation. Decay cannot do this job — `effectiveKnowledge` needs a `lastAttempt`, so an unseen word never fades. Without the boost, a newly added word ranked *below every word you had already practised* and was never selected at all: in a 200-word pool it appeared in 0 of 10 cards across 200 simulated sessions, versus 8 of 10 with it. The failure was silent, since the word looked present and unpaused everywhere in the UI.
- **Chapter 0 is not chapter 1.** `getInitialKnowledge` clamped chapter into `[1, 15]`, so user-added words (chapter 0) inherited chapter 1's prior of 70 — the app's *highest confidence* score — for words the user had explicitly just said they did not know. They now get `USER_ADDED_KNOWLEDGE` (30). Rows written before this fix keep the stored 70, which is why the unseen boost, not the prior, is what rescues them.
