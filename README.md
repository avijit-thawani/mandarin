...
3. **Only update README for major changes** (new features, new failure modes, architecture shifts, schema changes) — not minor bug fixes, style tweaks, or tuning knobs. Keep it on stable contracts and architecture; for volatile logic update module docstrings first, then README pointers. If a section feels outdated, propose doc updates in the same task.
