...
**A `vocabulary` row alone is invisible — `user_progress` is what the app reads.** `loadFromCloud` selects *from* `user_progress`, so a word with no progress row for that user is neither known nor unknown: it never appears in Vocabulary and can never be quizzed, with nothing in the UI hinting it exists. `saveToCloud` only upserts progress for words already in local state, so it never backfills. A raw SQL insert into `vocabulary` therefore needs a matching `user_progress` insert per user (`knowledge` 50, all four modalities at 70, `paused` false is the convention `addCustomWord` uses). 97 such orphans accumulated this way (Sep 18 2026) and were backfilled as known. **`addCustomWord` now writes both rows itself** — reusing a catalog row matched on word + pinyin (never word alone: 地 is both *de* and *dì*) and upserting progress with `ignoreDuplicates` so re-adding never resets earned knowledge. Leaving the progress row to the debounced sync is what let accepted Chat and trivia words vanish on the next app open.
...
- **Synonym disambiguation**: words sharing an English meaning (但是/可是 "but", 会/能 "can", 分/分钟 "minute", 些/少/一点儿) carry differentiated glosses so each meaning is unique. `hasCollision` is exact-string match on the answer-modality value, so disambiguation must live in the gloss, not the code — and it only catches *exact* duplicates, so near-duplicates ("day after tomorrow" vs "the day after tomorrow") sail through and are worse than identical ones.

**Glosses are quiz prompts, not dictionary entries** (audit applied to prod Sep 7 2026 — `content/vocab_fixes_20260828.py` is the idempotent source of truth; it rewrites the JSON and emits the SQL). A `meaning` may contain no hanzi (儿 shipped as "child; diminutive suffix (e.g., 一点儿, 那儿)", printing the answer inside a meaning→character prompt), no romanised pinyin (者 shipped as "-er", which reads as *ér* = 儿), and no jargon — "counting word for books", never "MW"/"measure word"/"marker". Distinguish senses rather than restating them: 少 "few; not many" vs 些 "several; some" are identical in English, and both sat at ~52% against an 89% baseline with a symmetric 9-for-9 swap between them. **Verify the gloss in Supabase, not the JSON** — the two drifted, and README plus `hsk1_vocabulary.json` documented 会/能 and 但是/可是 disambiguations that were never written to the DB, so the app served bare "can"/"but" for months. **Collisions regrow**: Chat-added words bypass this audit, and 可以 arrived mid-cleanup as "can, may, be allowed to", re-colliding with 能 until the pair was re-split three ways (会 learned skill / 能 physically able / 可以 permitted). Same-character rows with *different* pinyin (地 de vs dì) are distinct words, not duplicates — never dedupe on `word` alone.
...
- **Render her in every item branch** (MCQ, syntax, trivia) — scoping her to the MCQ branch made her disappear on every trivia and syntax item; she belongs to the session, not a question type.
...
- **Generate many, show few.** A fact is generated for every eligible question in the background, then `rankTrivia` picks the keepers. Quality is only knowable after generation, so curating beats showing whatever landed at a fixed interval. Tune `TRIVIA_SHOW_FRACTION`, not a user setting. **`TRIVIA_DEBUG_SHOW_ALL` must stay `false` on main**: the ranking effect returns early on it, so leaving it `true` shows every generated card and makes `TRIVIA_SHOW_FRACTION` and every later curation change inert (see `Known Failures`).
...
- The card's known-word context is capped at 120 words, biased toward words sharing a character with the focus word. **The verification payload is not capped, and not narrowed either** — the quiz pool is filtered by `paused` *and* the "for today" filter, so `fetchTrivia` scopes the three checks separately: atoms/owned characters from every unpaused word (derived from a chapter-filtered pool, they offer as "missing" a character the user owns in an excluded chapter), and the duplicate check against *every* row including paused ones. An unchecked word is still a word the user has, and offering it renders a card whose only state is a non-actionable "In vocab".
- **Accepting a suggestion for a word already in vocab enrols it** rather than inserting a duplicate row: `handleAddSuggestedWord` un-pauses the existing concept. `isKnownWord` therefore counts only unpaused words, so a paused suggestion still shows an Add button.
- **Every card is logged** to `trivia_log` (`src/lib/triviaLog.ts`), at *generation* time rather than display time: most cards are never shown, so judging quality from the survivors samples the wrong population. `shown` and `word_added` are patched on afterwards, making generated → shown → accepted measurable. Words are stored as text with no FK to `vocabulary`, so a vocab edit can never erase history. Logging failures are swallowed — analytics must never disturb a quiz — but a Postgres `42P01` is called out by name once per load, since "table never created" and "feature never ran" otherwise look identical from an empty table. **`20260918094500_create_trivia_log.sql` has never been confirmed applied to production**; until someone checks, assume the table may not exist there.
...
Historical incident (Feb 2, 2026): a migration changed ID references and added a FK path that orphaned/deleted large quiz history. Never repeat this class of failure. `.cursorrules` holds the binding rules; the essentials:

**Never** add FK constraints against remapped IDs without a mapping table, put `ON DELETE CASCADE` on a user-history path without full impact analysis, or recreate a live user table. **Always** back up prod first, show the exact SQL, explain the blast radius, wait for explicit approval, and test on a production-like copy or branch.

**Sensitive tables**: `quiz_attempts` (history + analytics + ML inputs), `user_progress` (current learning state), `user_settings` (behavioral preferences), `push_subscriptions` (per-device VAPID keys + reminder schedule), `chat_messages` (conversation content).
...
9. **Trivia curation never ran (Sep 2026)** — `TRIVIA_DEBUG_SHOW_ALL` shipped `true` in the feature's first commit and stayed true, and the ranking effect returns early on it. Every generated card was shown, `rankTrivia` never ran in production, and each later "improve the recommendations" commit landed on code a debug flag had already switched off. **Lesson**: a debug override that skips a whole code path must default to off on `main`; a flag left on makes every subsequent fix to that path unobservable.
10. **Accepted trivia words orphaned (Sep 2026)** — `addCustomWord` inserted the `vocabulary` row and left `user_progress` to the debounced sync. `loadFromCloud` replaces the local cache wholesale from `user_progress`, so a word added and not synced before the next app open disappeared from every surface while staying in the catalog. Now enrolled at add time. **Lesson**: a `vocabulary` row without `user_progress` is invisible — write both in the same action, never across a sync boundary.
